        ...
        <div>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">Archaeological field app</p>
                <h1 className="text-2xl font-black tracking-tight text-primary">ArchaeoLens</h1>
              </div>
            </div>
            <a href="/about" className="rounded-full border border-primary/20 bg-card px-3 py-2 text-xs font-semibold uppercase tracking-wide text-primary shadow-sm hover:bg-accent">
              Method
            </a>
          </div>

          <section className="mt-10">
            <p className="text-sm font-semibold uppercase tracking-wide text-muted-foreground">Photo-based artifact observation</p>
            ...
            <p className="mt-4 text-base leading-relaxed text-muted-foreground">
              Capture ceramics, lithics, coins, inscriptions, terracotta, metal, bone, or sculptural fragments and receive a cautious field record with diagnostic features, confidence level, and research references.
            </p>
          </section>

          <section className="mt-8 grid gap-3">
            <Feature icon={<ShieldCheck className="h-5 w-5" />} title="Authentic wording" text="Results separate visible evidence from dating, provenance, and final authentication." />
            <Feature icon={<Library className="h-5 w-5" />} title="Reference-backed" text="Links point to museum, heritage, and typology comparison resources." />
          </section>
        </div>
...
function Feature({ icon, title, text }: { icon: React.ReactNode; title: string; text: string }) {
  return (
    <div className="flex gap-3 rounded-xl border border-border bg-card p-4 shadow-sm">
      <div className="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-secondary text-primary">{icon}</div>
      <div>
